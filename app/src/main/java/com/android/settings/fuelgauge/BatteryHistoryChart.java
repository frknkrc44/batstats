...
public class BatteryHistoryChart extends View {
    ...
    public BatteryHistoryChart(Context context, AttributeSet attrs) {
        ...
        mBatteryWarnLevel = 15;
        mBatteryCriticalLevel = 5;

        mThinLineWidth = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                2, getResources().getDisplayMetrics());

        int accentColor = Utils.getColorAccentInt(getContext());
        ...
        if (mainTextAttrs.textColor == null)
            mainTextAttrs.textColor = getDefaultTextColor();
        headTextAttrs.textColor = mainTextAttrs.textColor;

        if (mainTextAttrs.typefaceIndex < 0)
            mainTextAttrs.typefaceIndex = SANS;
        headTextAttrs.typefaceIndex = mainTextAttrs.typefaceIndex;
    ...
    private ColorStateList getDefaultTextColor() {
        ...
        TypedArray arr = getContext().obtainStyledAttributes(value.data, new int[]{android.R.attr.textColorPrimary});
        return ColorStateList.valueOf(arr.getColor(0, 0xFF000000));
    }
    ...
    public void setStats(BatteryStats stats, Intent broadcast) {
        mStats = stats;
        if (mBatteryBroadcast != null) mBatteryBroadcast = broadcast;
    ...
    public void formatDuration(long millis, StringBuilder sb) {
        sb.append(Utils.formatElapsedTime(getContext(), millis));
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
            ...
            while (mStats.getNextHistoryLocked(rec) && i < N) {
                if (rec.isDeltaData()) {
                    ...
                    if (DEBUG) {
                        ...
                        sb.append("walloff=");
                        formatDuration(curWalltime - walltimeStart, sb);
                        sb.append(" wallchange=");
                        formatDuration(walltimeChange, sb);
                        sb.append(" x=");
                        sb.append(x);
        ...
        if (mTimeLabels.size() < 2) {
            ...
            mDurationString = Utils.formatElapsedTime(getContext(),
    ...
    void drawChart(Canvas canvas, int width, int height) {
        final boolean layoutRtl = getLayoutDirection() == LAYOUT_DIRECTION_RTL;
